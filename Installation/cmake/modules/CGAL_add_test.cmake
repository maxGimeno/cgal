...
function(cgal_add_compilation_test exe_name)
  ...
  add_test(NAME "compilation_of__${exe_name}"
    COMMAND ${TIME_COMMAND} "${CMAKE_COMMAND}" --build "${CMAKE_BINARY_DIR}" --target "${exe_name}" --config "$<CONFIG>" -- "/p:TrackFileAccess=false")
  ...
  if(NOT TEST check_build_system)
    add_test(NAME "check_build_system"
      COMMAND "${CMAKE_COMMAND}" --build "${CMAKE_BINARY_DIR}" --target "cgal_check_build_system" --config "$<CONFIG>" -- "/p:TrackFileAccess=false")
