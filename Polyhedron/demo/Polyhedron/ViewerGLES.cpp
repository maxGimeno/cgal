...
public:
  ...
  void draw_aux(bool with_names, ViewerGLES*);
...
void ViewerGLES::setScene(CGAL::Three::Scene_draw_interface* scene)
{
  d->scene = scene;
}

bool ViewerGLES::antiAliasing() const
{
  return d->antialiasing;
}

void ViewerGLES::setAntiAliasing(bool b)
{
  d->antialiasing = b;
  update();
...
void ViewerGLES::setTwoSides(bool b)
{
  d->twosides = b;
  update();
...
void ViewerGLES::setFastDrawing(bool b)
{
  d->inFastDrawing = b;
  update();
}

bool ViewerGLES::inFastDrawing() const
{
...
void ViewerGLES::mousePressEvent(QMouseEvent* event)
{
...
void ViewerGLES::keyPressEvent(QKeyEvent* e)
{
  if(!e->modifiers()) {
    if(e->key() == Qt::Key_T) {
...
void ViewerGLES::turnCameraBy180Degres() {
...
void Viewer_impl::draw_aux(bool with_names, ViewerGLES *viewer)
{
  if(scene == 0)
    return;
#if !ANDROID
...
bool ViewerGLES::moveCameraToCoordinates(QString s, float animation_duration) {
...
QString ViewerGLES::dumpCameraCoordinates()
{
  if(camera()->frame()) {
...
void ViewerGLES::attrib_buffers(int program_name) const {
...
void ViewerGLES::pickMatrix(float x, float y, float width, float height,
GLint viewport[4])
{
 //CGAL_GLdouble m[16];
 float sx, sy;
...
void ViewerGLES::beginSelection(const QPoint &point)
{
#if !ANDROID
...
void ViewerGLES::endSelection(const QPoint& point)
{
#if !ANDROID
...
void ViewerGLES::makeArrow(CGAL_GLdouble R, int prec, qglviewer::Vec from, qglviewer::Vec to, qglviewer::Vec color, AxisData &data)
{
...
QPointF posipoint;
bool ViewerGLES::event(QEvent *e)
{
...
QOpenGLShaderProgram* ViewerGLES::getShaderProgram(int name) const
{
  QString path = "";
#if ANDROID
  ...
  path = ":/shaders/resources/Shaders_desktop/";
    ...
    ViewerGLES* viewer = const_cast<ViewerGLES*>(this);
...
void ViewerGLES::wheelEvent(QWheelEvent* e)
{
