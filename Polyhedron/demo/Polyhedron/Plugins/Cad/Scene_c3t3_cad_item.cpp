...
typedef CGAL::AABB_triangulation_3_cell_primitive<EPICK,
                                                  C3t3_cad::Triangulation> Primitive;
...
class Scene_intersection_item : public CGAL::Three::Scene_item_rendering_helper
{
  Q_OBJECT
public :
  Scene_intersection_item(Scene_c3t3_cad_item* parent)
  :is_fast(false)
  {
    setParent(parent);
    alphaSlider = NULL;
  ...
  {
    qobject_cast<Scene_c3t3_cad_item*>(this->parent())->setColor(c);
  ...
  void draw(CGAL::Three::Viewer_interface* viewer) const Q_DECL_OVERRIDE
  {
    ...
    viewer->makeCurrent();
    const EPICK::Plane_3& plane = qobject_cast<Scene_c3t3_cad_item*>(this->parent())->plane();
    float shrink_factor = qobject_cast<Scene_c3t3_cad_item*>(this->parent())->getShrinkFactor();
  ...
  void addTriangle(const Tr_cad::Bare_point& pa, const Tr_cad::Bare_point& pb,
                   const Tr_cad::Bare_point& pc, const CGAL::Color color)
  {
    const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
    Geom_traits_cad::Vector_3 n = cross_product(pb - pa, pc - pa);
    n = n / CGAL::sqrt(n*n);
...
struct Scene_c3t3_cad_item_priv {
  ...
  Scene_c3t3_cad_item_priv(Scene_c3t3_cad_item* item)
    : item(item), c3t3(), mesh_domain(item->meshDomain())
  ...
  Scene_c3t3_cad_item_priv(const C3t3_cad& c3t3_, const Mesh_domain_cad& mesh_domain_, Scene_c3t3_cad_item* item)
    : item(item), c3t3(c3t3_)
    , mesh_domain(mesh_domain_)
    , frame(new ManipulatedFrame())
    , data_item_(NULL)
    , histogram_()
    , surface_patch_indices_()
  ...
  void fill_aabb_tree() {
    ...
    timer.start();
    tree.clear();
    for (Tr_cad::Finite_cells_iterator
           cit = c3t3.triangulation().finite_cells_begin(),
           end = c3t3.triangulation().finite_cells_end();
         cit != end; ++cit)
    {
      Tr_cad::Cell_handle ch = cit;
  ...
  void draw_triangle(const Tr_cad::Bare_point& pa,
                     const Tr_cad::Bare_point& pb,
                     const Tr_cad::Bare_point& pc) const;
  void draw_triangle_edges(const Tr_cad::Bare_point& pa,
                           const Tr_cad::Bare_point& pb,
                           const Tr_cad::Bare_point& pc) const;
  void draw_triangle_edges_cnc(const Tr_cad::Bare_point& pa,
                               const Tr_cad::Bare_point& pb,
                               const Tr_cad::Bare_point& pc) const;
  ...
  C3t3_cad c3t3;
  const Mesh_domain_cad& mesh_domain;
  bool is_grid_shown;
...
struct Set_show_tetrahedra {
  Scene_c3t3_cad_item_priv* priv;
  Set_show_tetrahedra(Scene_c3t3_cad_item_priv* priv) : priv(priv) {}
...
void Scene_c3t3_cad_item::common_constructor()
{
  compute_bbox();
...
Scene_c3t3_cad_item::Scene_c3t3_cad_item(const C3t3_cad& c3t3, const Mesh_domain_cad &md)
  : Scene_group_item("unnamed")
  , d(new Scene_c3t3_cad_item_priv(c3t3, md, this))
{
  d->reset_cut_plane();
  common_constructor();
}
...
const Scene_item*
Scene_c3t3_cad_item::data_item() const
{
  return d->data_item_;
}

void
Scene_c3t3_cad_item::set_data_item(const Scene_item* data_item)
{
...
void
Scene_c3t3_cad_item::data_item_destroyed()
{
  set_data_item(NULL);
}

const C3t3_cad&
Scene_c3t3_cad_item::c3t3() const {
  return d->c3t3;
}
...
void Scene_c3t3_cad_item::updateCutPlane()
...
void
Scene_c3t3_cad_item::c3t3_changed()
{
  // Update colors
  ...
  int max = 0;
  for (C3t3_cad::Cells_in_complex_iterator cit = this->c3t3().cells_in_complex_begin(),
  ...
  for (C3t3_cad::Facets_in_complex_iterator fit = this->c3t3().facets_in_complex_begin(),
    end = this->c3t3().facets_in_complex_end(); fit != end; ++fit)
  {
    max = (std::max)(max, 0// fit->first->surface_patch_index(// fit->second
                              //                               0 )
                        );
       d->surface_patch_indices_.insert(0// fit->first->surface_patch_index(// fit->second
                                         //                                0)
                                        );
     }
...
QPixmap
Scene_c3t3_cad_item::graphicalToolTip() const
{
  if (!d->histogram_.isNull())
  {
    return d->histogram_;
  }
  const_cast<Scene_c3t3_cad_item&>(*this).build_histogram();
  return d->histogram_;
}

std::vector<int>
create_histogram(const C3t3_cad& c3t3, double& min_value, double& max_value)
{
  Geom_traits_cad::Compute_approximate_dihedral_angle_3 approx_dihedral_angle
    = c3t3.triangulation().geom_traits().compute_approximate_dihedral_angle_3_object();
  Geom_traits_cad::Construct_point_3 wp2p
  ...
  for (C3t3_cad::Cells_in_complex_iterator cit = c3t3.cells_in_complex_begin();
    ...
    ++cit)
  {
    ...
    const Tr_cad::Bare_point& p0 = wp2p(cit->vertex(0)->point());
    const Tr_cad::Bare_point& p1 = wp2p(cit->vertex(1)->point());
    const Tr_cad::Bare_point& p2 = wp2p(cit->vertex(2)->point());
    const Tr_cad::Bare_point& p3 = wp2p(cit->vertex(3)->point());
...
void
Scene_c3t3_cad_item::build_histogram()
{
...
QColor
Scene_c3t3_cad_item::get_histogram_color(const double v) const
{
...
void
Scene_c3t3_cad_item::update_histogram()
{
  build_histogram();
}

void
Scene_c3t3_cad_item_priv::compute_color_map(const QColor& c)
{
...
Geom_traits_cad::Plane_3 Scene_c3t3_cad_item::plane(CGAL::qglviewer::Vec offset) const
{
  ...
  return Geom_traits_cad::Plane_3(n[0], n[1], n[2], -n * pos);
}

void Scene_c3t3_cad_item::compute_bbox() const {
  if (isEmpty())
    _bbox = Bbox();
  else {
    bool bbox_init = false;
    CGAL::Bbox_3 result;
    for (Tr_cad::Finite_vertices_iterator
...
QString Scene_c3t3_cad_item::toolTip() const {
...
void Scene_c3t3_cad_item::draw(CGAL::Three::Viewer_interface* viewer) const {
  if(!visible())
    return;
  Scene_c3t3_cad_item* ncthis = const_cast<Scene_c3t3_cad_item*>(this);
  if(!isInit(viewer))
    initGL(viewer);
  //viewer->makeCurrent();
  if ( getBuffersFilled() &&
...
void Scene_c3t3_cad_item::drawEdges(CGAL::Three::Viewer_interface* viewer) const {
  if(!visible())
    return;
...
void Scene_c3t3_cad_item::drawPoints(CGAL::Three::Viewer_interface * viewer) const
{
  if(!visible())
    return;
...
void Scene_c3t3_cad_item_priv::draw_triangle(const Tr_cad::Bare_point& pa,
                                         const Tr_cad::Bare_point& pb,
                                         const Tr_cad::Bare_point& pc) const
{
#undef darker
...
void Scene_c3t3_cad_item_priv::draw_triangle_edges(const Tr_cad::Bare_point& pa,
                                               const Tr_cad::Bare_point& pb,
                                               const Tr_cad::Bare_point& pc) const
{
#undef darker
...
void Scene_c3t3_cad_item_priv::draw_triangle_edges_cnc(const Tr_cad::Bare_point& pa,
                                                   const Tr_cad::Bare_point& pb,
                                                   const Tr_cad::Bare_point& pc) const
{
#undef darker
...
double Scene_c3t3_cad_item_priv::complex_diag() const {
...
void Scene_c3t3_cad_item::export_facets_in_complex()
{
  SMesh outmesh;
...
QMenu* Scene_c3t3_cad_item::contextMenu()
{
  const char* prop_name = "Menu modified by Scene_c3t3_cad_item.";
  ...
  if (!menuChanged) {
    ...
    connect(d->tet_Slider, &QSlider::valueChanged, this, &Scene_c3t3_cad_item::itemChanged);
...
void Scene_c3t3_cad_item_priv::initializeBuffers(CGAL::Three::Viewer_interface *viewer)
{
  //vao containing the data for the facets
  {
    item->getTriangleContainer(Scene_c3t3_cad_item::C3t3_faces)->initializeBuffers(viewer);
    item->getTriangleContainer(Scene_c3t3_cad_item::C3t3_faces)->setFlatDataSize(
          positions_poly_size);
  ...
  //vao containing the data for the lines
  {
    item->getEdgeContainer(Scene_c3t3_cad_item::C3t3_edges)->initializeBuffers(viewer);
    item->getEdgeContainer(Scene_c3t3_cad_item::C3t3_edges)->setFlatDataSize(
          positions_lines_size);
  }
  //vao containing the data for the points
  {
    item->getPointContainer(Scene_c3t3_cad_item::C3t3_points)->initializeBuffers(viewer);
    item->getPointContainer(Scene_c3t3_cad_item::C3t3_points)->setFlatDataSize(
          positions_lines_size);
  ...
  {
    item->getEdgeContainer(Scene_c3t3_cad_item::CNC)->initializeBuffers(viewer);
    item->getEdgeContainer(Scene_c3t3_cad_item::CNC)->setFlatDataSize(
  ...
  {
    item->getEdgeContainer(Scene_c3t3_cad_item::Grid_edges)->initializeBuffers(viewer);
    item->getEdgeContainer(Scene_c3t3_cad_item::Grid_edges)->setFlatDataSize(
          positions_grid.size());
  }
}
...
void Scene_c3t3_cad_item_priv::computeIntersection(const Primitive& cell)
{
  Geom_traits_cad::Construct_point_3 wp2p
    = c3t3.triangulation().geom_traits().construct_point_3_object();

  typedef unsigned char UC;
  Tr_cad::Cell_handle ch = cell.id();
  QColor c = this->colors_subdomains[ch->subdomain_index()].lighter(50);

  const Tr_cad::Bare_point& pa = wp2p(ch->vertex(0)->point());
  const Tr_cad::Bare_point& pb = wp2p(ch->vertex(1)->point());
  const Tr_cad::Bare_point& pc = wp2p(ch->vertex(2)->point());
  const Tr_cad::Bare_point& pd = wp2p(ch->vertex(3)->point());
...
struct ComputeIntersection {
  Scene_c3t3_cad_item_priv& item_priv;

  ComputeIntersection(Scene_c3t3_cad_item_priv& item_priv)
    : item_priv(item_priv)
  {}
...
void Scene_c3t3_cad_item_priv::computeIntersections(CGAL::Three::Viewer_interface* viewer)
{
  const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
  ...
  positions_lines.clear();
  positions_barycenter.clear();
  const Geom_traits_cad::Plane_3& plane = item->plane(offset);
...
void Scene_c3t3_cad_item_priv::computeSpheres()
{
      ...
      vit != end; ++vit)
  {
    ...
    typedef Tr_cad::Vertex_handle Vertex_handle;
        ...
        vvit != end; ++vvit)
    {
      ...
      if(Geom_traits_cad::Sphere_3(wp2p(vit->point()),
    ...
    const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
    Tr_cad::Bare_point center(wp2p(vit->point()).x() + offset.x,
                          wp2p(vit->point()).y() + offset.y,
                          wp2p(vit->point()).z() + offset.z);
    float radius = vit->point().weight() ;
    typedef unsigned char UC;
    spheres->add_sphere(Geom_traits_cad::Sphere_3(center, radius),
                        0,
...
void Scene_c3t3_cad_item_priv::computeElements()
{
  if(!alphaSlider)
   {
     alphaSlider = new QSlider(::Qt::Horizontal);
  ...
  //The facets
  {
    ...
    for (C3t3_cad::Facet_iterator
      fit = c3t3.facets_begin(),
      end = c3t3.facets_end();
    fit != end; ++fit)
    {
      const Tr_cad::Cell_handle& cell = fit->first;
      const int& index = fit->second;
      const Tr_cad::Bare_point& pa = wp2p(cell->vertex((index + 1) & 3)->point());
      const Tr_cad::Bare_point& pb = wp2p(cell->vertex((index + 2) & 3)->point());
      const Tr_cad::Bare_point& pc = wp2p(cell->vertex((index + 3) & 3)->point());

      QColor color = colors[0// cell->surface_patch_index(index)
                                  ];
    ...
    for(C3t3_cad::Triangulation::Cell_iterator
        cit = c3t3.triangulation().finite_cells_begin(),
        end = c3t3.triangulation().finite_cells_end();
        cit != end; ++cit)
    {
      if(!c3t3.is_in_complex(cit))
      {
        ...
        if(!has_far_point)
        {
          const Tr_cad::Bare_point& p1 = wp2p(cit->vertex(0)->point());
          const Tr_cad::Bare_point& p2 = wp2p(cit->vertex(1)->point());
          const Tr_cad::Bare_point& p3 = wp2p(cit->vertex(2)->point());
          const Tr_cad::Bare_point& p4 = wp2p(cit->vertex(3)->point());
...
bool Scene_c3t3_cad_item::load_binary(std::istream& is)
{
...
void
Scene_c3t3_cad_item_priv::reset_cut_plane() {
  const CGAL::Three::Scene_item::Bbox& bbox = item->bbox();
...
void
Scene_c3t3_cad_item::setColor(QColor c)
{
  color_ = c;
...
void Scene_c3t3_cad_item::show_grid(bool b)
{
  d->is_grid_shown = b;
  contextMenu()->findChild<QAction*>("actionShowGrid")->setChecked(b);
  itemChanged();
}
void Scene_c3t3_cad_item::show_spheres(bool b)
{
  if(is_valid())
  {
...
void Scene_c3t3_cad_item::show_intersection(bool b)
{
...
void Scene_c3t3_cad_item::show_cnc(bool b)
{
  if(is_valid())
  {
    d->cnc_are_shown = b;
...
void Scene_c3t3_cad_item::reset_intersection_item()
{
  d->intersection = NULL;
}

void Scene_c3t3_cad_item::reset_spheres()
{
  d->spheres = NULL;
}
CGAL::Three::Scene_item::ManipulatedFrame* Scene_c3t3_cad_item::manipulatedFrame() {
  if(d)
    return d->frame;
  else
    return NULL;
...
void Scene_c3t3_cad_item::setPosition(float x, float y, float z) {
   const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
  d->frame->setPosition(x+offset.x, y+offset.y, z+offset.z);
}

bool Scene_c3t3_cad_item::has_spheres()const { return d->spheres_are_shown;}

bool Scene_c3t3_cad_item::has_grid()const { return d->is_grid_shown;}

bool Scene_c3t3_cad_item::has_cnc()const { return d->cnc_are_shown;}

bool Scene_c3t3_cad_item::has_tets()const { return d->intersection; }

void Scene_c3t3_cad_item::setNormal(float x, float y, float z) {
  d->frame->setOrientation(x, y, z, 0.f);
}

void Scene_c3t3_cad_item::copyProperties(Scene_item *item)
{
  Scene_c3t3_cad_item* c3t3_item = qobject_cast<Scene_c3t3_cad_item*>(item);
  if(!c3t3_item)
    return;
   const CGAL::qglviewer::Vec offset = Three::mainViewer()->offset();
...
bool Scene_c3t3_cad_item::is_valid() const
{
  return d->is_valid;
}
void Scene_c3t3_cad_item::set_valid(bool b)
{
  d->is_valid = b;
}
float Scene_c3t3_cad_item::getShrinkFactor() const
{
  return float(d->tet_Slider->value())/100.0f;
}

bool Scene_c3t3_cad_item::eventFilter(QObject *, QEvent *event)
{
...
bool Scene_c3t3_cad_item::keyPressEvent(QKeyEvent *event)
{
...
QString Scene_c3t3_cad_item::computeStats(int type)
{
  Geom_traits_cad::Construct_point_3 wp2p
  ...
  if(!d->computed_stats)
  {
    ...
    QVector<int> sub_ids;
    for (C3t3_cad::Cells_in_complex_iterator cit = d->c3t3.cells_in_complex_begin();
  ...
  switch (type)
  {
  case Scene_c3t3_cad_item_priv::MIN_EDGES_LENGTH:
    return QString::number(d->min_edges_length);
  case Scene_c3t3_cad_item_priv::MAX_EDGES_LENGTH:
    return QString::number(d->max_edges_length);
  case Scene_c3t3_cad_item_priv::MEAN_EDGES_LENGTH:
    return QString::number(d->mean_edges_length);
  case Scene_c3t3_cad_item_priv::MIN_DIHEDRAL_ANGLE:
    return QString::number(d->min_dihedral_angle);
  case Scene_c3t3_cad_item_priv::MAX_DIHEDRAL_ANGLE:
    return QString::number(d->max_dihedral_angle);
  case Scene_c3t3_cad_item_priv::MEAN_DIHEDRAL_ANGLE:
    return QString::number(d->mean_dihedral_angle);
  case Scene_c3t3_cad_item_priv::NB_SPHERES:
    return QString::number(d->nb_spheres);
  case Scene_c3t3_cad_item_priv::NB_CNC:
    return QString::number(d->nb_cnc);
  case Scene_c3t3_cad_item_priv::NB_VERTICES:
    return QString::number(d->nb_vertices);
  case Scene_c3t3_cad_item_priv::NB_TETS:
    return QString::number(d->nb_tets);
  case Scene_c3t3_cad_item_priv::SMALLEST_RAD_RAD:
    return QString::number(d->smallest_radius_radius);
  case Scene_c3t3_cad_item_priv::SMALLEST_EDGE_RAD:
    return QString::number(d->smallest_edge_radius);
  case Scene_c3t3_cad_item_priv::BIGGEST_VL3_CUBE:
    return QString::number(d->biggest_v_sma_cube);
  case Scene_c3t3_cad_item_priv::NB_SUBDOMAINS:
...
CGAL::Three::Scene_item::Header_data Scene_c3t3_cad_item::header() const
{
...
void Scene_c3t3_cad_item::invalidateOpenGLBuffers()
{
...
void Scene_c3t3_cad_item::resetCutPlane()
{
  if(!d)
    return;
 d->reset_cut_plane();
...
void Scene_c3t3_cad_item::itemAboutToBeDestroyed(Scene_item *item)
{
...
void Scene_c3t3_cad_item::on_spheres_color_changed()
{
  if(!d->spheres)
    return;
...
float Scene_c3t3_cad_item::alpha() const
{
  if(!d->alphaSlider)
    return 1.0f;
...
void Scene_c3t3_cad_item::setAlpha(int alpha)
{
  if(!d->alphaSlider)
    d->computeElements();
...
QSlider* Scene_c3t3_cad_item::alphaSlider() {
  if(!d->alphaSlider)
    d->computeElements();
  return d->alphaSlider;
}

void Scene_c3t3_cad_item::initializeBuffers(Viewer_interface *v) const
{
  const_cast<Scene_c3t3_cad_item*>(this)->d->initializeBuffers(v);
}

void Scene_c3t3_cad_item::computeElements()const
{
  QApplication::setOverrideCursor(Qt::WaitCursor);
  const_cast<Scene_c3t3_cad_item*>(this)->d->computeElements();
...
void Scene_c3t3_cad_item::newViewer(Viewer_interface *viewer)
{
...
const Mesh_domain_cad &Scene_c3t3_cad_item::meshDomain() const
{
    return d->mesh_domain;
}
#include "Scene_c3t3_cad_item.moc"
